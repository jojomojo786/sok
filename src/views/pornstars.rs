...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PornstarsIndexView {
    pub cards: Vec<PornstarCardView>,
    pub sort_links: Vec<PornstarSortLink>,
...
impl Default for PornstarsIndexView {
    fn default() -> Self {
        Self {
            cards: Vec::new(),
            sort_links: Vec::new(),
...
impl PornstarsIndexView {
    pub fn build(
        ...
        let cards: Vec<PornstarCardView> = entity_cards
            .iter()
            .map(|c| PornstarCardView {
                slug: c.slug.clone(),
                display_name: c.display_name.clone(),
                thumb_url: c.thumb_url(cdn_base),
                profile_url: format!("/pornstar/{}", c.slug),
...
fn entity_sort_links(page: u32, sort: &SortKey, hd: HdFilter) -> Vec<PornstarSortLink> {
    let listing = ListingKind::EntityIndex(EntityIndexKind::Pornstars);
    ...
    .into_iter()
    .map(|(label, key)| PornstarSortLink {
        label,
...
mod tests {
    ...
    fn entity_card(slug: &str, name: &str) -> EntityIndexCard {
        EntityIndexCard {
            ...
            thumb_path: format!("fox-images/pornstars/{slug}.jpg"),
            video_count: 42,
        }
    }
    ...
    fn card_html_preserves_dom_hooks() {
        ...
        assert!(view.grid_html.contains("/pornstar/angela-white"));
        assert!(view.grid_html.contains("count-videos"));
        assert!(view.grid_html.contains("#camera-svg"));
        assert!(view.grid_html.contains("Angela White"));
    }

    #[test]
    fn page_nav_uses_pornstars_prefix() {
        ...
        assert!(view.page_nav_html.contains("/pornstars/2"));
    }
}
